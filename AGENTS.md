...
機能ごとに専門化されたビューモジュール。各ビューは独立したファイルとして構成されています：
...
- **モデルテスト**: `moneybook/tests/models/periodicDataTests.py`
  - モデルのCRUD操作、ソート順の確認
- **ビューテスト**: 
  - `moneybook/tests/views/periodicViewTests.py`: 一覧表示、未認証時302リダイレクト
  - `moneybook/tests/views/periodicEditViewTests.py`: 編集画面表示（GET）、フォームPOST更新処理（POST）
